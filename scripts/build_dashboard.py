...
def main() -> int:
    ...
    # GC can live on multiple pages; try the race overview first, then fall back to stage page.
    gc_candidates = [fetch_html(r_url), stage_html]
    ...
    page = render_dashboard(ctx=ctx, stage_df=stage_df, gc_df=gc_df)
